...
if (isset($_POST['action']) && $_POST['action'] == 'registration') {
    ...
    if ($result->num_rows > 0) {
        ...
        if ($insertQuery->execute()) {
            $success_message = "Gracias por registrarte con nosotros. Puedes iniciar sesión.";
            echo $success_message;
            exit();
        } else {
            $error_message = "Problema al insertar registro!";
        }
        $insertQuery->close();
        $conn->close();
...
if (isset($_POST['action']) && $_POST['action'] == 'login') {
    ...
    if ($result->num_rows > 0) {
        ...
        $error_message = "Invalido el email Id o password!";
    } // endElse
    $conn->close();
